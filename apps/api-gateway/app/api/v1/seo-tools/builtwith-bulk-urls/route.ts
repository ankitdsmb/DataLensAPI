...
const builtWithBulkPolicy = createToolPolicy({
  timeoutMs: 10000,
...
export const POST = withScrapingHandler({ policy: builtWithBulkPolicy }, async (req: Request) => {
  const body = await readJsonBody<Record<string, unknown>>(req, builtWithBulkPolicy);
  requireAllowedFields(body, ['url', 'urls']);
  const urls = collectUrlInputs(body, builtWithBulkPolicy);

  const results = [];

  for (const url of urls) {
    const { html, $ } = await fetchHtmlDocument(url, { timeoutMs: builtWithBulkPolicy.timeoutMs });
