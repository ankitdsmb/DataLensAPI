...
const simpleSeoAuditPolicy = createToolPolicy({
  timeoutMs: 10000,
...
export const POST = withScrapingHandler({ policy: simpleSeoAuditPolicy }, async (req: Request) => {
  const body = await readJsonBody<Record<string, unknown>>(req, simpleSeoAuditPolicy);
  const urls = collectUrlInputs(body, simpleSeoAuditPolicy);
  ...
  for (const url of urls) {
    const { $ } = await fetchHtmlDocument(url, { timeoutMs: simpleSeoAuditPolicy.timeoutMs });
