...
const techStackPolicy = createToolPolicy({
  timeoutMs: 12000,
...
export const POST = withScrapingHandler({ policy: techStackPolicy }, async (req: Request) => {
  const body = await readJsonBody<Record<string, unknown>>(req, techStackPolicy);
  const urls = collectUrlInputs(body, techStackPolicy);

  const results = [];

  for (const url of urls) {
    const { html, $ } = await fetchHtmlDocument(url, { timeoutMs: techStackPolicy.timeoutMs });
