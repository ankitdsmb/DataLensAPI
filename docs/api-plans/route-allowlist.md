...
| `/api/v1/seo-tools/snapify-capture-screenshot-save-pdf` | `queued-browser` | Keep out of the **free-tier** profile, but allow as an **authenticated beta** outside free-tier mode; it now renders real screenshot/PDF artifacts in a browser worker, enforces public-host validation plus page/artifact budgets, and uses submitter-bound status/artifact reads with a 6-hour job TTL and 2-hour artifact retention window. |
