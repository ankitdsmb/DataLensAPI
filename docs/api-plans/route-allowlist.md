...
| `/api/v1/seo-tools/snapify-capture-screenshot-save-pdf` | `queued-browser` | Keep out of the **free-tier** profile, but allow as an **authenticated beta** outside free-tier mode; it now renders real screenshot/PDF artifacts in a browser worker, enforces public-host validation plus page/artifact budgets, and uses authenticated-only status/artifact reads with a 6-hour job TTL and 2-hour artifact retention window. |
| `/api/v1/seo-tools/youtube-rank-checker` | `queued-simulated` | Keep out of the **free-tier** profile, but allow as a **credentialed preview** outside free-tier mode; it now enforces supported YouTube video URLs, uses multi-strategy YouTube search evidence parsing plus a browser-assisted DOM fallback with provenance, and keeps authenticated-only status/artifact reads with a 12-hour job TTL and 6-hour artifact retention window. |
