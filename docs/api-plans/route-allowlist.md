...
| `/api/v1/seo-tools/youtube-rank-checker` | `queued-simulated` | Keep internal-only as an **internal preview**; the current worker now uses multi-strategy YouTube search evidence parsing with provenance, but still degrades to simulation when live evidence is unavailable. |
