...
| `/api/v1/seo-tools/snapify-capture-screenshot-save-pdf` | `queued-browser` | Keep internal-only as an **internal preview**; the current worker now renders real screenshot/PDF artifacts in a browser worker, and preview job status/artifact reads are authenticated-only with a 6-hour job TTL and 2-hour artifact retention window. |
| `/api/v1/seo-tools/youtube-rank-checker` | `queued-simulated` | Keep internal-only as an **internal preview**; the current worker now uses multi-strategy YouTube search evidence parsing with provenance, and preview job status/artifact reads are authenticated-only with a 12-hour job TTL and 6-hour artifact retention window. |
