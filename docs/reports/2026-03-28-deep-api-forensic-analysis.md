...
| `/api/v1/seo-tools/snapify-capture-screenshot-save-pdf` | Full-page screenshots + PDFs. | 2/5 | ~20% | Real async job submission exists and returns durable job/artifact envelopes. | Current worker emits synthetic screenshot/PDF artifact records instead of rendered files. | Replace synthetic capture output with real screenshot/PDF generation before public launch. | Not launch-ready on free hosting; keep internal or preview-only until execution is real. | No route-specific GitHub delta observed beyond the repo-wide branch lag. |
