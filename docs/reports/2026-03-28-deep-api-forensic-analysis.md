...
| `/api/v1/seo-tools/dns-lookup-forward-and-reverse-a-mx-txt-dmarc-ptr` | DNS/Reverse DNS lookup. | 2/5 | ~25% | Implements a local heuristic or convenience transform without a real provider. | Many are useful as lite helpers but not strong enough for the current product promise. | Relabel as lite utilities or back them with stronger engines/providers. | Excellent technical fit for free hosting. | No route-specific GitHub delta observed beyond the repo-wide branch lag. |
| `/api/v1/seo-tools/domain-availability-checker` | Domain availability check. | 2/5 | ~25% | Implements a local heuristic or convenience transform without a real provider. | Many are useful as lite helpers but not strong enough for the current product promise. | Relabel as lite utilities or back them with stronger engines/providers. | Excellent technical fit for free hosting. | No route-specific GitHub delta observed beyond the repo-wide branch lag. |
| `/api/v1/seo-tools/domain-availability-expiry-whois-dns-ip-asn-70-tld` | Availability + WHOIS + DNS + ASN. | 2/5 | ~25% | Implements a local heuristic or convenience transform without a real provider. | Many are useful as lite helpers but not strong enough for the current product promise. | Relabel as lite utilities or back them with stronger engines/providers. | Excellent technical fit for free hosting. | No route-specific GitHub delta observed beyond the repo-wide branch lag. |
| `/api/v1/seo-tools/domain-checker` | Availability + market value; suggestions. | 2/5 | ~25% | Implements a local heuristic or convenience transform without a real provider. | Many are useful as lite helpers but not strong enough for the current product promise. | Relabel as lite utilities or back them with stronger engines/providers. | Excellent technical fit for free hosting. | No route-specific GitHub delta observed beyond the repo-wide branch lag. |
| `/api/v1/seo-tools/domain-inspector` | DNS/WHOIS/HTTP/SSL for domains. | 2/5 | ~25% | Implements a local heuristic or convenience transform without a real provider. | Many are useful as lite helpers but not strong enough for the current product promise. | Relabel as lite utilities or back them with stronger engines/providers. | Excellent technical fit for free hosting. | No route-specific GitHub delta observed beyond the repo-wide branch lag. |
...
| Endpoint | Promise | Strength | Coverage | Current Logic | Biggest Gap | Best-Ever Upgrade | Free-Tier Fit | GitHub Note |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `/api/v1/seo-tools/openpagerank-bulk-checker` | Bulk OpenPageRank scores. | 1/5 | ~5% | Normalizes input and returns a truthful provider-template contract with `pending_api_key` output. | No live provider data exists yet. | Implement a real provider adapter or keep it explicit as an internal/template route. | Not ready until credentials and provider integration exist. | No route-specific GitHub delta observed beyond the repo-wide branch lag. |
| `/api/v1/seo-tools/rentcast` | Rent estimates for US properties. | 1/5 | ~10% | Builds a lookup URL and returns a truthful provider-template contract with `pending_api_key`. | No live provider data exists yet. | Either integrate the provider or keep it explicit as a template-only/internal route. | Not ready until credentials and provider integration exist. | No route-specific GitHub delta observed beyond the repo-wide branch lag. |
...
| `/api/v1/seo-tools/snapify-capture-screenshot-save-pdf` | Full-page screenshots + PDFs. | 3/5 | ~40% | Real async job submission exists and now captures live page HTML evidence plus durable job/artifact envelopes. | The current worker still stops at HTML evidence and metadata; screenshot/PDF binary rendering is not implemented yet. | Evolve this into a true browser/PDF worker while preserving the current evidence-capture mode as a lighter fallback. | Better than before, but still not free-tier launch-ready; keep internal or preview-only until rendering is real. | No route-specific GitHub delta observed beyond the repo-wide branch lag. |
...
| `/api/v1/seo-tools/traffic-booster` | Quick traffic boosts for analytics/tests. | 2/5 | ~15% | Real async job submission exists and returns durable job/artifact envelopes. | Current worker only produces projection-style planning output and the route is policy-quarantined. | Keep disabled for public launch and replace projection logic only if a compliant internal use case survives review. | Poor public fit: policy-conflicted, abuse-prone, and not compliant for public launch. | No route-specific GitHub delta observed beyond the repo-wide branch lag. |
...
| `/api/v1/seo-tools/youtube-rank-checker` | Check video ranks by keyword. | 3/5 | ~45% | Real async job submission exists and now attempts lightweight YouTube search evidence collection before returning durable job/artifact envelopes. | The current worker still relies on fragile public HTML parsing and falls back to deterministic simulation when live evidence cannot be collected. | Promote into a canonical rank-tracker family with stronger evidence capture, retries, and explicit degraded-mode telemetry. | Better than before, but still not free-tier launch-ready; keep internal or preview-only until execution hardening improves. | No route-specific GitHub delta observed beyond the repo-wide branch lag. |
