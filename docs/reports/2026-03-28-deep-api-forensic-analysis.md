...
| `/api/v1/seo-tools/youtube-rank-checker` | Check video ranks by keyword. | 2/5 | ~20% | Real async job submission exists and returns durable job/artifact envelopes. | Current worker returns deterministic simulated rank output rather than collected ranking evidence. | Implement inside a canonical rank-tracker family with real search evidence before public launch. | Not launch-ready on free hosting; keep internal or preview-only until execution is real. | No route-specific GitHub delta observed beyond the repo-wide branch lag. |
