...
- `npm run smoke-tests`
  - verifies `youtube-rank-checker` rejects unsupported non-YouTube hosts, supports credentialed preview submission, authenticated job/artifact access, provenance-bearing async job completion, and the strengthened live-evidence path
