...
| `/api/v1/seo-tools/snapify-capture-screenshot-save-pdf` | `queued-browser` | Strengthened into stable authenticated beta | Excluded from the free-tier subset | Real async job contract now renders screenshot/PDF artifacts in a browser worker, rejects private-host targets by default, enforces page/artifact budgets with HTML-evidence fallback for oversized pages, and runs in non-free-tier mode with submitter-bound authenticated beta access. |
| `/api/v1/seo-tools/youtube-rank-checker` | `queued-simulated` | Strengthened into credentialed preview-only posture | Excluded from the free-tier subset | Real async job contract now enforces supported YouTube video URLs, uses multi-strategy YouTube search evidence parsing plus a browser-assisted DOM fallback with provenance, retains deterministic fallback when live evidence still cannot be collected, and keeps submitter-bound preview access outside free-tier mode. |
...
- `npm run smoke-tests`
  - verifies `youtube-rank-checker` rejects unsupported non-YouTube hosts, supports credentialed preview-only submission, enforces submitter-bound job/artifact access, and completes with provenance-bearing live evidence or honest fallback
  - verifies `snapify-capture-screenshot-save-pdf` stable authenticated-beta submission, enforces submitter-bound job/artifact access, delivers browser-rendered artifacts, and safely falls back to HTML evidence for oversized pages
